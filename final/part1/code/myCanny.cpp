...
canny::canny(CImg<double> srcImg, int gsize, double sigma, double highThresh, double lowThresh) {
  // load the image
  img = srcImg;
...
/*
  删除短边缘 
 */
CImg<double> canny::edgeDelete(int shortLen, int longLen) {
  deleted = CImg<double>(width, height);
  // initial
  cimg_forXY(deleted, x, y) {
    deleted(x, y) = linked(x, y);
  }
  ...
  cimg_forXY(deleted, x, y) {
    if (deleted(x, y) == 255 && SignMap(x, y) == 0) {
      int count = 0;
      ...
      while (!temp.empty()) {
        ...
        for (int i = nowX - 1; i < nowX + 2; i++) {
          for (int j = nowY - 1; j < nowY + 2; j++) {
            // 搜索八邻域
            if (i >= 0 && i < width && j >= 0 && j < height) {
              if (deleted(i, j) == 255 && SignMap(i, j) == 0) {
                count++;
                SignMap(i, j) = 1;
                temp.push(make_pair(i, j));
      ...
      // 若长度不在限定范围内
      if (count < shortLen || count > longLen) {
        while (!PointSet.empty()) {
          // 将预先保存在容器中的边上的点删除
          deleted(PointSet.front().first, PointSet.front().second) = 0;
          PointSet.pop();
        }
      }
    }
