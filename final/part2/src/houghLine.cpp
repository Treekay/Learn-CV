...
houghLine::houghLine(CImg<double> _img, int Xsize, int Ysize, int thresh, string _resPath) {
  resPath = _resPath;
  edgeImg = _img;
  width = edgeImg.width();
...
void houghLine::drawLines() {
  /* 画出投票数较高的直线 */
  cout << "The Line Equation: " << endl;            ///////debug
...
void houghLine::detectPoints() {
  ...
  img.save((resPath + "3_houghImg.bmp").c_str());
}
